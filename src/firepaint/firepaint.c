...
static void
fireAddPoint (CompScreen * s, int x, int y)
{
	FIRE_SCREEN (s);
	...
	if (fs->grabIndex)
	{
		if (fs->pointsSize < fs->numPoints + 1)
		{
			fs->points =
				realloc (fs->points,
						 (fs->pointsSize + NUM_ADD_POINTS) * sizeof (XPoint));
...
static void
firePreparePaintScreen (CompScreen * s, int time)
{
	int i;
	float size = 4;
	...
	{
		float max_new =
			MIN (fs->ps.numParticles,
				 fs->numPoints * 2) * ((float)time / 50.0) * (1.05 -
															  firepaintGetFireLife
															  (s));
		Particle *part;
		float rVal;
		int rVal2;
		...
		for (i = 0; i < fs->ps.numParticles && max_new > 0; i++)
		{
			...
			if (part->life <= 0.0f)
			{
				...
				part->life = 1.0f;
				part->fade = (rVal * (1 - firepaintGetFireLife (s)) + (0.2f * (1.01 - firepaintGetFireLife (s))));	// Random Fade Value
...
static void
fireHandleEvent (CompDisplay * d, XEvent * event)
{
	CompScreen *s;
...
static Bool
fireInitDisplay (CompPlugin * p, CompDisplay * d)
{
	FireDisplay *fd;
...
static void
fireFiniDisplay (CompPlugin * p, CompDisplay * d)
{
	FIRE_DISPLAY (d);
...
static Bool
fireInitScreen (CompPlugin * p, CompScreen * s)
{
	FireScreen *fs;
...
static void
fireFiniScreen (CompPlugin * p, CompScreen * s)
{
	FIRE_SCREEN (s);
...
static int
fireGetVersion (CompPlugin * plugin, int version)
{
	return ABIVERSION;
}
